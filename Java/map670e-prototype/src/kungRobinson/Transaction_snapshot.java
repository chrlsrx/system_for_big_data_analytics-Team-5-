	...
	private boolean validate(ArrayList<Transaction_snapshot> other_transactions) {
		for (int i = 0; i < other_transactions.size(); i++) {
			...
			if (transaction.get_ts_start_validate() > 0){
				test_3 = transaction.get_ts_start_validate() < this.ts_start_validate && this.list_intersect(transaction.get_write_set(), this.read_set).size() == 0 && this.list_intersect(transaction.get_write_set(), this.write_set).size() == 0;
			}
