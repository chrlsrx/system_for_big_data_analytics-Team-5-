...
public class Transaction_snapshot_Single_thread {
	...
	public Transaction_snapshot_Single_thread(final int id, int w_id, Database db_original) {
		this.id = id;
		this.phase = "";
	...
	private boolean validate(ArrayList<Transaction_snapshot_Single_thread> other_transactions) {
		for (int i = 0; i < other_transactions.size(); i++) {
			Transaction_snapshot_Single_thread transaction = other_transactions.get(i);
	...
	public boolean apply_next(ArrayList<Transaction_snapshot_Single_thread> other_transactions) {
		if (this.is_finished()) {
			return true;
		}
