...
public class Transaction_optimistic_cc {
	...
	private boolean validate(ArrayList<Transaction_optimistic_cc> other_transactions) {
		for (int i = 0; i < other_transactions.size(); i++) {
			...
			// We only test this transaction for the other transactions that started BEFORE current one
			if (transaction.get_ts_start_read() < this.ts_start_read){
				...
				if (transaction.get_ts_start_validate() > 0){
					// This test is complex. We break it down in 3 subtets.
					boolean subtest_3_1 = transaction.get_ts_start_validate() < this.ts_start_validate;
					boolean subtest_3_2 = this.list_intersect(transaction.get_write_set(), this.read_set).size() == 0;
					boolean subtest_3_3 = this.list_intersect(transaction.get_write_set(), this.write_set).size() == 0;
				...
				if (transaction.is_finished()){
					...
					boolean subtest_2_1 = transaction.get_ts_start_finish() < System.currentTimeMillis();
					boolean subtest_2_2 = this.list_intersect(transaction.get_write_set(), this.read_set).size() == 0;
