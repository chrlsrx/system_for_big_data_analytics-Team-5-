...
public class NewOrderTransactionLock implements Runnable {
	...
	public NewOrderTransactionLock(int transaction_id, int w_id, Database d, LockManager lockmanager, Scheduler sch, DataGeneration data) {
	...
	public synchronized boolean runTransaction() throws InterruptedException {
		...
		// Create a ReadLock, store the lock, and then try to put the lock on the warehouse 
		ReadLock read0 = new ReadLock(this.transaction_id, this.db, this.lockmanager, fake_wh.hashCode(), fake_wh, Types.WAREHOUSE, this.ts);
		...
		if (status == Status.ABORT) {
			...
			this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ; // We remove all the lock previously acquired
		...
		WriteLock update0 = new UpdateLock(this.transaction_id, this.db, this.lockmanager, fake_d, Types.DISTRICT, this.ts);
		...
		ReadLock read1 = new ReadLock(this.transaction_id, this.db, this.lockmanager, c_code, fake_c, Types.CUSTOMER, this.ts);
		...
		for (int i = 0; i < number_items-1; i++) {
			...
			WriteLock update1 = new UpdateLock(this.transaction_id, this.db, this.lockmanager, fake_i, Types.ITEM, this.ts);
			...
			if (it == null) {
				...
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				this.lockmanager.remove_locks(fake_i, false, this.transaction_id) ;
				continue; // We go to the next item after releasing the lock.
				//return false ;
			}
			...
			WriteLock update2 = new UpdateLock(this.transaction_id, this.db, this.lockmanager, fake_s, Types.STOCK, this.ts);
			...
			if (s == null) {
				...
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				this.lockmanager.remove_locks(fake_i, false, this.transaction_id) ;
				this.lockmanager.remove_locks(fake_s, false, this.transaction_id) ;
				continue;
				//return false ;
			}
			...
			WriteLock write2 = new WriteLock(this.transaction_id, this.db, this.lockmanager, ol, Types.ORDER_LINE, this.ts);
...
/*
		...
		if (status == Status.ABORT) {
			this.sch.retry(this) ;
			this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
			System.out.println("Transaction " + this.transaction_id + " is aborting 57.");
			return false ;
		} else if (status == Status.WAIT) {
		...
		cnt++;
		//this.lockmanager.remove_locks(w, true, this.transaction_id) ; // Lock on the Warehouse4   
		...
		this.store_lock(d_code, true) ;
		status = read2.applyLock() ;
		if (status == Status.ABORT) {
			this.sch.retry(this) ;
			this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
			System.out.println("Transaction " + this.transaction_id + " is aborting 84.");
			return false ;
		} else if (status == Status.WAIT) {
			System.out.println("Transaction " + this.transaction_id + " waits a bit 84");
			TimeUnit.MILLISECONDS.sleep(200);
			status = read2.applyLock() ;
		}
		...
		double d_tax = d.get_d_tax();
		int d_next_o_id = d.get_d_next_o_id();
		//this.lockmanager.remove_locks(d, true, this.transaction_id) ; // Lock on the Order
		
		WriteLock write0 = new WriteLock(this.transaction_id, this.db, this.lockmanager, d, Types.DISTRICT, this.ts);
		this.store_lock(d.hashCode(), false) ;
		status = write0.applyLock() ;
		if (status == Status.ABORT) {
			this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
			this.sch.retry(this) ;
			System.out.println("Transaction " + this.transaction_id + " is aborting 117.");
			return false ;
		} else if (status == Status.WAIT) {
			System.out.println("Transaction " + this.transaction_id + " waits a bit 117");
			TimeUnit.MILLISECONDS.sleep(200);
			status = write0.applyLock() ;
		}
		write0.apply();
		...
		d.inc_next_o_id() ;
		cnt++;
		//this.lockmanager.remove_locks(d, false, this.transaction_id) ; // Lock on the Order
		...
		if (status == Status.ABORT) {
			System.out.println("Transaction " + this.transaction_id + " is aborting 112.");
			this.sch.retry(this) ;
			this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
			return false ;
		} else if (status == Status.WAIT) {
		...
		if (status == Status.ABORT) {
			...
			System.out.println(this.lockmanager);
			this.sch.retry(this) ;
			System.out.println("Transaction " + this.transaction_id + " is aborting 151.");
			return false ;
		} else if (status == Status.WAIT) {
			System.out.println("Transaction " + this.transaction_id + " waits a bit");
			TimeUnit.MILLISECONDS.sleep(200);
		...
		if (status == Status.ABORT) {
			this.sch.retry(this) ;
			//this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
			System.out.println("Transaction " + this.transaction_id + " is aborting 171.");
			return false ;
		} else if (status == Status.WAIT) {
			System.out.println("Transaction " + this.transaction_id + " waits a bit");
			TimeUnit.MILLISECONDS.sleep(200);
		...
		for (int i = 0; i < number_items-1; i++) {
			...
			this.store_lock(code_item, true) ;
			status = read4.applyLock() ;
			if (status == Status.ABORT) {
				this.sch.retry(this) ;
				//this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				System.out.println("Transaction " + this.transaction_id + " is aborting 210.");
				return false ;
			} else if (status == Status.WAIT) {
				System.out.println("Transaction " + this.transaction_id + " waits a bit");
				TimeUnit.MILLISECONDS.sleep(200);
			...
			int gogogo = code_stock ;
			this.store_lock(code_stock, true) ;
			status = read5.applyLock() ;
			if (status == Status.ABORT) {
				this.sch.retry(this) ;
				System.out.println("Transaction " + this.transaction_id + " is aborting 240.");
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				return false ;
			} else if (status == Status.WAIT) {
				System.out.println("Transaction " + this.transaction_id + " waits a bit");
			...
			this.store_lock(fake_s.hashCode(), false) ;
			status = write3.applyLock() ;
			if (status == Status.ABORT) {
				this.sch.retry(this) ;
				System.out.println("Transaction " + this.transaction_id + " is aborting 266.");
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				return false ;
			} else if (status == Status.WAIT) {
				System.out.println("Transaction " + this.transaction_id + " waits a bit");
			...
			this.store_lock(fake_i.hashCode(), false) ;
			status = write4.applyLock() ;
			if (status == Status.ABORT) {
				this.sch.retry(this) ;
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				System.out.println("Transaction " + this.transaction_id + " is aborting 307.");
				return false ;
			} else if (status == Status.WAIT) {
				System.out.println("Transaction " + this.transaction_id + " waits a bit");
				TimeUnit.MILLISECONDS.sleep(200);
			...
			if (status == Status.ABORT) {
				this.sch.retry(this) ;
				this.lockmanager.remove_all_locks(this.transaction_id, this.objs_hash, this.isReads) ;
				System.out.println("Transaction " + this.transaction_id + " is aborting 341.");
				return false ;
			} else if (status == Status.WAIT) {
				System.out.println("Transaction " + this.transaction_id + " waits a bit");
				TimeUnit.MILLISECONDS.sleep(200);
