...
public class LockManager {
	...
	public boolean add_lock(Object o, boolean l_read, int transaction_hash) {
		...
		if (l_read) {
			...
			// trans.)
			Lock new_lock = new Lock(o, l_read, transaction_hash);
			keys.add(new_lock);
	...
	public boolean remove_locks(Object o, boolean l_read, int transaction_hash) {
		...
		i = 0;
		// For the reads, we can also remove duplicates (if exist), so no "removed" condition
		while (i <= reads.size()) {
	...
	public boolean isXLocked(Object o) {
		int hash = o.hashCode() ;
	...
	public boolean isSLocked(Object o) {
		int hash = o.hashCode() ;
		boolean already_locked = false ;
